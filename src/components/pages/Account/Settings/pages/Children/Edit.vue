<template>
  <modal name="modal-edit-child" :click-to-close="false" height="auto" :adaptive="true" :reset="true"
    ...
    <div class="vmodal-body p-4">
      ...
      <form id="formChildren" @submit.prevent="save" v-if="child">
...
<script>
    ...
    export default {
        ...
        watch: {
            currentChild(value) {
                if (value !== null) {
                    this.child = this.helpers.clone(value)
                    this.child.firstname = this.child.firstname.toLowerCase().capitalize()
                    this.child.lastname = this.child.lastname.toLowerCase().capitalize()
                }
            }
        },
        ...
        methods: {
            ...
            save() {
                ...
                data.birthday = this.helpers.moment(this.child.birthday).format('YYYY-MM-DD')
                if (!this.isValidDate(this.child.birthday))
                    return this.notValidDateError()

                this.api.put('/children', this.child.id, data).then(
                    () => this.$emit('updated'),
